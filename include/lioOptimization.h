...
struct Measurements
{
    ...
    cv::Mat image;
    std::vector<std::tuple<cv::Mat, int, int32_t>> semantic_masks; // modified for semantics
...
public:
    ...
    //modified: semantics
    //Store decoded semantic masks
    std::vector<std::tuple<cv::Mat, int, int32_t>> semantic_masks;
    int getSemantic(const double &u, const double &v, int &instance_id);
};
...
enum IMAGE_TYPE{RGB8 = 1, COMPRESSED = 2, CUSTOM = 3};

#include <oneformer_pkg/segmentation_voxblox.h>
#include <nlohmann/json.hpp>

// RLE structure for mask decoding
struct RLE {
    int h, w, m;
...
private:
    ...
    // Semantic mask buffer (stores decoded masks with labels)
    std::queue<std::vector<std::tuple<cv::Mat, int, int32_t>>> semantic_mask_buffer;
    // Format: vector of (mask, semantic_label, instance_id)
    
    // Subscriber for custom semantic message
    ros::Subscriber sub_semantic_msg;
    
    // Helper functions
    RLE rleFrString(const std::string &s, int h, int w);
    cv::Mat rleDecode_Optimized(const RLE &rle);
    //void semanticMsgHandler(const oneformer_pkg::segmentation_voxblox::ConstPtr& msg);
...
// ADE20K color palette for semantic visualization
inline cv::Scalar semanticLabelToColor(int label)
{
    static const std::vector<cv::Scalar> ADE20K_COLORMAP = {
        {128, 128, 128}, {180, 120, 120}, {6, 230, 230}, {80, 50, 50}, {4, 200, 3},
    ...
    if (label <= 0) {
        return cv::Scalar(255, 255, 255);  // White for unlabeled/background
    }
    
    size_t index = (label - 1) % ADE20K_COLORMAP.size();
    return ADE20K_COLORMAP[index];
}