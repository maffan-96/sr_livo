...
void rgbPoint::updateSemanticLabel(int new_label)
{
    if (new_label == 0) return;  // Ignore background
    
    // Voting-based fusion
    semantic_histogram[new_label]++;
    N_semantic++;
    
    // Find most frequent label (majority vote)
    int max_count = 0;
    int best_label = new_label;
    for (const auto &[label, count] : semantic_histogram) {
        if (count > max_count) {
            max_count = count;
            best_label = label;
        }
