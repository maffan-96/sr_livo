...
int cloudFrame::getSemantic(const double &u, const double &v, int &instance_id)
{
...
void cloudFrame::release()
{
    ...
    // Semantic mask release (added on top of SR-LIVO's cloudFrame)
...
lioOptimization::lioOptimization()
{
    ...
    if (image_type == RGB8)
        ...
        {
        std::cout << "The `image type` is custom for semantics**." << std::endl;
        sub_semantic_msg = nh.subscribe<oneformer_pkg::segmentation_voxblox>(
        segmentation_topic, 20, &lioOptimization::semanticMsgHandler, this);
    }
...
{	
    ...
    nh.param<std::string>("common/segmentation_topic", segmentation_topic, "/segmentations");
    ...
    else if(str_temp == "CUSTOM") image_type = CUSTOM;
...
// Decode RLE string to RLE structure
RLE lioOptimization::rleFrString(const std::string &s, int h, int w) {
    int m = 0, p = 0;
    ...
    while (p < s.size()) {
        ...
        counts.push_back(static_cast<uint32_t>(x));
        m++;
    }
...
// Optimized RLE decoder (Fortran order - column-major)
cv::Mat lioOptimization::rleDecode_Optimized(const RLE &rle) {
    cv::Mat mask(rle.h, rle.w, CV_8UC1, cv::Scalar(0));

    uint8_t pixel_value = 0;
    int index = 0, row = 0, col = 0;
    int total_pixels = rle.h * rle.w;

    for (uint32_t count : rle.counts) {
        if (index >= total_pixels) break;

        while (count-- > 0 && index < total_pixels) {
            row = index % rle.h;  // Fortran order
            col = index / rle.h;
            mask.ptr<uint8_t>(row)[col] = pixel_value;
            index++;
        }
        pixel_value ^= 255;  // Toggle between 0 and 255
    }
...
// Handler for custom semantic message
void lioOptimization::semanticMsgHandler(const oneformer_pkg::segmentation_voxblox::ConstPtr& msg)
{
    ...
    try {
    cv_ptr = cv_bridge::toCvCopy(msg->original_image, sensor_msgs::image_encodings::BGR8);
    ...
    std::vector<std::tuple<cv::Mat, int, int32_t>> mask_data;
    mask_data.clear();
    
    for (size_t i = 0; i < msg->masks.size() && i < msg->detections.detections.size(); ++i) {
        try {
            if (msg->masks[i].empty()) continue;
            
            // Parse JSON-encoded RLE
            json mask_json = json::parse(msg->masks[i]);
            int height = mask_json["size"][0];
            int width = mask_json["size"][1];
            std::string counts = mask_json["counts"];
            
            // Decode RLE
            RLE rle = rleFrString(counts, height, width);
            cv::Mat mask = rleDecode_Optimized(rle);
            
            if (mask.empty()) continue;
            
            // Extract label and instance ID
            auto& detection = msg->detections.detections[i];
            int semantic_label = detection.results[0].id + 1;  // +1 to match ADE20K indexing
            int32_t instance_id = static_cast<int32_t>(msg->instance_ids[i]);
            
            mask_data.emplace_back(mask, semantic_label, instance_id);
            
        } catch (const std::exception& e) {
            ROS_ERROR("Error decoding mask %zu: %s", i, e.what());
        }
    }
    
    // Push to buffers
    // Push to buffers (make a deep copy because toCvShare shares memory)
    img_buffer.push(cv_ptr->image.clone());
    time_img_buffer.push(t);
    semantic_mask_buffer.push(mask_data);
}
...
std::vector<Measurements> lioOptimization::getMeasurements()
{
    ...
    while (true)
    {
        if (imu_buffer.empty() || img_buffer.empty() || semantic_mask_buffer.empty() || 
            time_img_buffer.empty() || point_buffer.empty())
            return measurements;
        ...
        else
        {
            ...
            measurement.semantic_masks = semantic_mask_buffer.front(); //modified
            ...
            semantic_mask_buffer.pop(); //modified
...
void lioOptimization::pubSemanticPoints(ros::Publisher &pub_cloud_semantic, cloudFrame *p_frame)
{
    pcl::PointCloud<pcl::PointXYZRGB> points_semantic_vec;
    sensor_msgs::PointCloud2 ros_points_msg;
    ...
    for (int i = 0; i < img_pro->map_tracker->rgb_points_vec.size(); i++)
    {
        ...
        // Skip points without enough observations
        if (p_point->N_rgb < map_options.pub_point_minimum_views) continue;
        
        // Skip points without semantic label
        if (!p_point->hasSemanticLabel()) continue;

        pcl::PointXYZRGB semantic_point;

        semantic_point.x = p_point->getPosition()[0];
        semantic_point.y = p_point->getPosition()[1];
        semantic_point.z = p_point->getPosition()[2];

        // Map semantic label to color
        cv::Scalar color = semanticLabelToColor(p_point->getSemanticLabel());
        semantic_point.r = static_cast<uint8_t>(color[2]);  // BGR to RGB
        semantic_point.g = static_cast<uint8_t>(color[1]);
        semantic_point.b = static_cast<uint8_t>(color[0]);

        points_semantic_vec.push_back(semantic_point);
    }

    pcl::toROSMsg(points_semantic_vec, ros_points_msg);

    ros_points_msg.header.stamp = ros::Time().fromSec(p_frame->time_sweep_end);
    ros_points_msg.header.frame_id = "camera_init";

    pub_cloud_semantic.publish(ros_points_msg);
}

void lioOptimization::threadPubSemanticPoints()
{
    int last_pub_map_index = -1000;
    int sleep_time_after_pub = 10;
    int number_of_points_per_topic = 1000;
    ...
    while (1)
    {
        ...
        points_semantic_vec.reserve(points_size);
        ...
        for (int i = 0; i < points_size; i++)
        {
            ...
            semantic_point.r = static_cast<uint8_t>(color[2]);
            semantic_point.g = static_cast<uint8_t>(color[1]);
            semantic_point.b = static_cast<uint8_t>(color[0]);
        ...
        if (points_semantic_vec.empty()) continue;
